...
// DatapointRingBuffer is a ring buffer that supports inserting and reading
...
// backing array of this datapoint.  This means that if the buffer wraps around,
...
func NewDatapointRingBuffer(size int) *DatapointRingBuffer {
	return &DatapointRingBuffer{
		// Preallocate the buffer to its maximum length
		buffer:    make([]*datapoint.Datapoint, size),
		bufferLen: size,
	}
}

// Add an Datapoint:datapoint.Datapoint to the buffer. It will overwrite any existing element in the
