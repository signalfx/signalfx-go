...
//go:generate sh -c "sed -e /go:generate/d -e s/datapoint_ring/span_ring/g -e s/datapoint/trace/g -e s/Datapoint/Span/g $GOFILE | gofmt -s > span_buffer_test.go"
...
func TestDatapointBuffer(t *testing.T) {
	t.Parallel()

	t.Run("Return elements added", func(t *testing.T) {
		t.Parallel()
		buffer := NewDatapointRingBuffer(100)

		for i := 0; i < buffer.Size(); i++ {
			require.Equal(t, buffer.UnprocessedCount(), i)
			overwrote := buffer.Add(&datapoint.Datapoint{
