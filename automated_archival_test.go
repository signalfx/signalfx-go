...
func TestGetSettings(t *testing.T) {
	...
	var CreatedUpdatedTs = 1674598662022
	...
	assert.Equal(t, "P60D", result.LookbackPeriod, "Lookback period doesn't match")
	assert.Equal(t, "P30D", result.GracePeriod, "Grace period doesn't match")
	assert.Equal(t, CreatedUpdatedTs, result.Created, "'Created' timestamp doesn't match")
	assert.Equal(t, CreatedUpdatedTs, result.LastUpdated, "'Last Updated' timestamp doesn't match")
