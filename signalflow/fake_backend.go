...
func (f *FakeBackend) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		f.handleMessage(ctx, in, textMsgs, binMsgs)
	}
}

func (f *FakeBackend) handleMessage(ctx context.Context, message map[string]interface{}, textMsgs chan<- string, binMsgs chan<- []byte) {
	f.Lock()
	defer f.Unlock()
	...
	case "execute":
		...
		log.Printf("Executing SignalFlow program %s", program)
